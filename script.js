...
const firebaseConfig = {
    apiKey: "AIzaSyDUIyk4gKjkodIbo9L-6d1EzbP4Y0eDAmo",
    authDomain: "parqueadero-pppb.firebaseapp.com",
    projectId: "parqueadero-pppb",
    storageBucket: "parqueadero-pppb.firebasestorage.app",
    messagingSenderId: "8252614455",
    appId: "1:8252614455:web:d5917541790006a6e09cb2"
};

async function subirPDF(blob, nombreArchivo) {
...
📅 Inicio: ${data.inicio.toLocaleDateString('es-CO', { day: 'numeric', month: 'long', year: 'numeric' })}
📅 Fin: ${data.fin.toLocaleDateString('es-CO', { day: 'numeric', month: 'long', year: 'numeric' })}
📅 Pago: ${fechaPago}
...
async function loadClientMensualidades(placa) {
    ...
    querySnapshot.forEach(doc => 
    {
        ...
        row.innerHTML = `
            ...
            <td data-label="Tipo">${data.tipoVehiculo}</td>
            <td data-label="Pagador">${nombrePagaDisplay}</td>
            <td data-label="Valor">${valorMensualidadDisplay}</td>
            <td data-label="Inicio">${inicio.toLocaleDateString('es-CO')}</td>
            <td data-label="Fin">${fin.toLocaleDateString('es-CO')}</td>
            <td data-label="Pago">${fechaPagoDisplay}</td>
        ...
        row.querySelector('.pdf-btn').addEventListener('click', () => generarComprobanteMensualidad(data));
    });
    }
...
async function generarComprobanteMensualidad(data){
    try {
        ...
        let colorEstado = [0, 0, 0]; // default

            if (estado === "Activa") colorEstado = [46, 204, 113]; // verde
            if (estado === "Vencida") colorEstado = [231, 76, 60]; // rojo
            if (estado === "Pendiente") colorEstado = [241, 196, 15]; // amarillo

        const fechaPago = data.fechaPago 
            ? data.fechaPago.toDate().toLocaleDateString('es-CO') 
            : 'N/A';

        const valor = data.valorMensualidad 
            ? data.valorMensualidad.toLocaleString('es-CO', { style: 'currency', currency: 'COP' }) 
            : 'N/A';
        const valorNumero = data.valorMensualidad || 0;
        const valorLetras = numToWords(valorNumero);
        ...
        doc.text("PARQUEADERO PPPB", 105, 20, null, null, "center");

        doc.setFontSize(12);
        doc.setTextColor(0,0,0);
        doc.text("Comprobante de Mensualidad", 105, 28, null, null, "center");
        ...
        doc.text(`Generado: ${new Date().toLocaleString()}`, 15, 45);

        // 🔷 CAJA
        doc.setDrawColor(220);
        doc.roundedRect(15, 50, 180, 80, 3, 3);
        ...
        linea("Placa:", data.placa);
        linea("Tipo:", data.tipoVehiculo);
        linea("Pagador:", data.nombrePersonaPaga || 'N/A');
        linea("Inicio:", inicio.toLocaleDateString('es-CO', { day: 'numeric', month: 'long',year: 'numeric'}));
        linea("Fin:", fin.toLocaleDateString('es-CO', { day: 'numeric', month: 'long', year: 'numeric' }));
        linea("Pago:", data.fechaPago 
    ? data.fechaPago.toDate().toLocaleDateString('es-CO', { 
        day: 'numeric', 
        month: 'long', 
        year: 'numeric' 
...
doc.roundedRect(15, 120, 180, 15, 5, 5, 'F');

// 🔷 VALOR
doc.setTextColor(255, 255, 255);
doc.setFontSize(14);
doc.text(`VALOR: ${valor}`, 105, 128, null, null, "center");

// 🔷 VALOR EN LETRAS (DENTRO DEL CUADRO)
doc.setFontSize(10);
doc.text(`(${valorLetras})`, 105, 133, null, null, "center");
...
doc.roundedRect(15, 160, 60, 12, 3, 3, 'F');

// TEXTO ESTADO
doc.setTextColor(255, 255, 255);
doc.setFontSize(10);
doc.text(estado.toUpperCase(), 45, 168, null, null, "center");

// RESTAURAR COLOR
doc.setTextColor(0, 0, 0);


// 🔷 FIRMA (VISIBLE Y BIEN UBICADA)
// 🔷 FIRMA CENTRADA
// 🔷 FIRMA (CENTRADA ABAJO)
try {
    doc.addImage(firmaBase64, "PNG", 75, 205, 60, 30);
} catch (e) {
    console.log("Error firma");
}

// 🔷 LÍNEA DE FIRMA (RECTA Y CENTRADA)
doc.line(70, 235, 140, 235);

// 🔷 TEXTO FIRMA
doc.setFontSize(10);
doc.text("Firma de recibido", 95, 242);


// 🔷 MENSAJE FINAL (CENTRADO)
doc.setFontSize(11);
doc.text("¡Gracias por su pago!", 108, 260, null, null, "center");

const nombreArchivo = `Mensualidad_${data.placa}.pdf`;
...
window.open(link, "_blank");
        ...
        alert("Error al subir PDF (revisa consola F12)");
    }
}
...
function numToWords(num) {
    ...
    return s.trim() + ' PESOS M/CTE'; // Añadido espacio antes de 'PESOS'
}
